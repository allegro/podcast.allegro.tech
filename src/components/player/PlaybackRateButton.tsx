...
import {AudioPlayerContextType} from "@/components/AudioProvider";

const playbackRates = [
    {
        value: 1,
        icon: function PlaybackIcon(props: SVGProps<SVGSVGElement>) {
            return (
                <svg
                    aria-hidden="true"
                    viewBox="0 0 16 16"
...
export function PlaybackRateButton({player}: { player: AudioPlayerContextType }) {
    const [playbackRate, setPlaybackRate] = useState(playbackRates[0])

    return (
        <button
            type="button"
            className="relative flex h-6 w-6 items-center justify-center rounded-md text-slate-500 hover:bg-orange-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2"
            onClick={() => {
